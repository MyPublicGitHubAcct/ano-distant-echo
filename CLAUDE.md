...
Known gaps: Wow/flutter depth is exposed via the Mod Depth knob (rates are fixed); wow/flutter CV and diffusion CV inputs not exposed in VCV Rack (TODO 20n); Delay ping-pong mode not exposed as a panel control — stereo always uses independent mode (TODO 20m); Distortion Type and Clip Shape have no CV inputs (TODO 20d).
