...
- [ ] **`make test` should run Python pytest** — The current `make test` recipe ends with `$(MAKE) compare`; append `uv run --project python pytest` so Python unit tests (`python/tests/test_effects.py`) are included in the single-command verification pipeline; Python tests are currently orphaned from `make test` and only run manually.
- [ ] **`make lint` target** — Add a `lint` phony target to the `Makefile`: `ruff check python/ && ruff format --check python/`; `ruff check` catches style and logic issues while `ruff format --check` enforces formatting without modifying files; currently only `ruff check` is mentioned in `CLAUDE.md` and neither check is part of the build pipeline; add `lint` as a dependency of a future `ci` target.
- [ ] **GitHub Actions CI workflow** — Create `.github/workflows/ci.yml` that runs on every push and pull request: install Homebrew tools (`cmake ninja catch2 uv ruff`), run `make lint`, run `uv run --project python pytest`, configure CMake and build `effects_tests`, run `ctest --output-on-failure`, run `make golden && make compare`; use `macos-latest` runner; the absence of CI means regressions that `make test` would catch locally can silently land on `main`.
...
- [ ] **JUCE: Report oversampler group delay via `setLatencySamples()`** — The 4× polyphase FIR in `Oversampler` introduces a linear-phase group delay of `(N_TAPS − 1) / (2 × L) = 127 / 8 ≈ 15.875` samples at the original sample rate; the current `prepareToPlay()` never calls `setLatencySamples()`, so DAWs cannot compensate and the overdrive output is ~16 samples late relative to other tracks when recording; add `static constexpr int groupDelaySamples() { return (N_TAPS - 1) / (2 * L); }` to `Oversampler`; in `DistantEchoProcessor::prepareToPlay()`, call `setLatencySamples(odBypass ? 0 : Oversampler::groupDelaySamples())`; update the latency whenever `od_bypass` changes by calling `setLatencySamples()` from `processBlock()` when the bypass state toggles; add a Catch2 test or pluginval run confirming the reported latency equals the oversampler filter group delay.
- [ ] **JUCE: Real-time safety — `CabinetIR::setType()` must not allocate on the audio thread** — `CabinetIR::setType()` calls `buildActiveIR()` which calls `activeIR.resize()` (heap allocation); `setType()` is driven by `od_cabinet_type` read in `processBlock()`, so every cabinet-type knob change causes a heap allocation mid-stream; move IR selection to a lock-free SPSC queue: write the new `CabinetType` from `processBlock()` into an `std::atomic<int> pendingCabinetType`, check and rebuild in `prepareToPlay()` or in a dedicated non-RT thread; alternatively, pre-build all three resampled IRs in `prepare()` and swap between pre-allocated buffers using an `std::atomic<int> activeCabinetIndex`; the current behavior is a real-time safety violation that can cause audio dropouts under memory pressure.
...
### 21n. APVTS parameter groups for DAW browser organization

- [ ] The current `createParameterLayout()` returns a flat list of ~22 parameters; in DAW automation lanes and plugin browsers the parameters appear as an undifferentiated list with no indication of which belong to Overdrive vs. Delay; wrap the parameter set in two `juce::AudioProcessorParameterGroup` objects: `"overdrive_grp"` containing all `od_*` parameters and `"delay_grp"` containing all `dl_*` parameters; pass the groups as children of the root `ParameterLayout`; no behavioral change — only the DAW-side grouping and display name changes; verify that all existing `apvts.getRawParameterValue("od_...")` calls still compile (parameter IDs are unchanged) and that pluginval still passes after the structural change; update `juce-plugin/PluginProcessor.cpp` `createParameterLayout()` accordingly.

### 21o. MIDI learn / MIDI CC mapping

- [ ] Add MIDI CC learn so hardware expression pedals, volume pedals, and controller footswitches can drive any plugin parameter without DAW automation; JUCE provides `AudioProcessorValueTreeState::addParameterListener()` but not a built-in MIDI-learn mechanism — implement a lightweight layer: add `std::unordered_map<int, juce::String> midiCCMap` (CC number → parameter ID) stored in the plugin state alongside the APVTS XML via `getStateInformation()`/`setStateInformation()`; in `processBlock()`, iterate `midiMessages` for `MidiMessage::isController()` events and call `apvts.getParameter(id)->setValueNotifyingHost(normalized)` for any mapped CC; in the editor, add right-click "Assign MIDI CC…" context menus on every knob and button; display the assigned CC number in the label when active; expose `dl_time` mapped to CC 4 (foot controller) as a factory default so users can control delay time from a volume pedal immediately on load.
...
### 22s. Add documentation plots for extended Delay parameters

- [ ] `docs/Delay.md` references wow/flutter modulation, diffusion bloom, ducking, and tape saturation but has no corresponding plots in `docs/img/`; the current `generate_docs_plots.py` covers only the feedback LP response and echo decay envelope; add the following plots via new functions in `generate_docs_plots.py`: (1) `delay-diffusion-impulse.png` — impulse response with `diffusion=0` vs. `diffusion=1`, showing the allpass bloom that spreads the leading edge over ~30 ms; (2) `delay-wow-flutter.png` — spectrogram (or instantaneous frequency vs. time) of a sustained sine with `wowDepth=4ms` and `flutterDepth=1ms`, showing the periodic pitch deviation; (3) `delay-duck-envelope.png` — the dry input envelope, the resulting wet attenuation gain, and the output level on one plot, demonstrating the side-chain timing at `duckThreshold=−20 dBFS, duckDepth=0.8`; (4) `delay-tape-sat-spectrum.png` — frequency spectrum of the feedback path output with and without tape saturation at `feedback=0.7`, showing harmonic enrichment above 2 kHz; link all four images in the relevant `docs/Delay.md` subsections.
...
### 26f. `Oversampler` — add `groupDelaySamples()` static method

- [ ] The oversampler's linear-phase group delay is `(N_TAPS − 1) / (2 × L) ≈ 15.875` samples but this value is currently buried in comments; add `static constexpr int groupDelaySamples() { return (N_TAPS - 1) / (2 * L); }` to `Oversampler.h` so callers — particularly the JUCE processor's `setLatencySamples()` call (TODO 11) and any future compensation buffer — can use the authoritative value rather than hardcoding 16; add a Catch2 test in `tests/test_oversampler.cpp` (or a new test section in an existing file) that processes a unit impulse through the full upsample + downsample chain and confirms the output peak lands within ±1 sample of `Oversampler::groupDelaySamples()`.

### 26g. `DelayLine` — add `size()` accessor

- [ ] `DelayLine<T>` exposes no way to query the allocated buffer length at runtime; callers that need to guard against reading past the end (e.g., `CabinetIR`'s circular convolution index, `Delay`'s crossfade counter relative to `activeTimeSamples`) compute the safe range independently and without a shared source of truth; add `int size() const { return static_cast<int>(buf.size()); }` to `DelayLine.h`; update any existing bounds checks in `CabinetIR.h` and `Delay.h` that hardcode or recompute the buffer size to call `delayLine.size()` instead; add a Catch2 test confirming `size()` returns the value passed to `resize()`.
...
## 27. Distribution and Signing

These items are prerequisites for sharing the plugin with users who are not building from source.

### 27a. macOS code signing for VST3, AU, and Standalone

- [ ] Apple Gatekeeper blocks unsigned binaries since macOS 10.15 Catalina; unsigned VST3/AU builds will show "cannot be opened because the developer cannot be verified" on any Mac where the user did not explicitly approve the binary; sign all three formats using a Developer ID Application certificate: pass `-DCMAKE_XCODE_ATTRIBUTE_CODE_SIGN_IDENTITY="Developer ID Application: ..."` when configuring with CMake (or add `set_target_properties(...PROPERTIES XCODE_ATTRIBUTE_CODE_SIGN_IDENTITY ...)` in `juce-plugin/CMakeLists.txt`); sign the `.vst3` bundle, the `.component` (AU), and the `.app` (Standalone) with `codesign --deep --force --verify --timestamp --options runtime`; document the signing steps and required certificate in `README.md` under a "Distribution" heading.

### 27b. macOS notarization for Gatekeeper

- [ ] Code signing alone is not sufficient for distribution via download since macOS 12+ Monterey requires notarization for executables from the internet; after signing (27a), submit the binaries to Apple's notary service via `xcrun notarytool submit <file> --apple-id ... --team-id ... --password ... --wait`; staple the notarization ticket with `xcrun stapler staple <file>`; create a `make notarize` Makefile target that signs and notarizes all three plugin formats in sequence; store the Apple ID and team ID as environment variables (`APPLE_ID`, `APPLE_TEAM_ID`, `APPLE_APP_PASSWORD`) so the step is scriptable in CI without hardcoding credentials; document the one-time setup (app-specific password, Keychain storage) in `README.md`.

### 27c. VCV Rack `plugin.json` — add module descriptions and tags

- [ ] The `plugin.json` for both modules has only minimal tags (`['Distortion', 'Effect']` for Overdrive and `['Delay', 'Effect']` for Delay) and no `description` field; the VCV Rack module browser shows the description under the module name in the search results — without it, users see only the slug; add a `"description"` field to each module entry: `"Guitar overdrive with 5 distortion types, 4× oversampling, cabinet IR, and pick sensitivity"` for Overdrive and `"Tape-style delay with wow/flutter, diffusion, ducking, self-oscillation, and stereo modes"` for Delay; also add `"Filter"` to Overdrive tags (the cabinet IR and tone controls qualify) and `"Modulation"` to Delay tags (wow/flutter modulation); the VCV Library rejects submissions with empty or placeholder descriptions, so this is a prerequisite for any future Library submission.
